...
def send_orders_and_create_route(token, resource_id, unit_id, vehicle_name, df_grouped, tf, tt, warehouse_choice):
    try:
        ...
        df_grouped = df_grouped.sort_values('Distance_From_Warehouse').reset_index(drop=True)
        ...
        coord_map[wh_name] = {'y': wh_lat, 'x': wh_lon}
        ...
        for resp in optimized_orders:
            ...
            coords = coord_map.get(order_name, {'y': wh_lat, 'x': wh_lon})
        ...
        route_orders.append({
            ...
            "n": wh_name,
            "p": {"ut": 0, "rep": True, "w": "0", "c": "0",
                  "r": {"vt": last_visit_time + 600, "ndt": 60, "id": route_id,
